...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Tạo Đơn hàng Mới</h1>
        ...
        <form action="{{ route('admin.orders.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="UserID" id="UserID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->UserID }}" {{ old('UserID') == $user->UserID ? 'selected' : '' }}>{{ $user->name ?? 'Người dùng không tên' }} (ID: {{ $user->UserID }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="OrderDate" id="OrderDate" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('OrderDate', date('Y-m-d')) }}" required>
            </div>

            <div class="mb-4">
                <label for="TotalAmount" class="block text-gray-700 text-sm font-bold mb-2">Tổng tiền:</label>
                <input type="number" step="0.01" name="TotalAmount" id="TotalAmount" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('TotalAmount', 0) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="ShippingAddress" id="ShippingAddress" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('ShippingAddress') }}</textarea>
            </div>

            <div class="mb-4">
                <label for="PaymentMethod" class="block text-gray-700 text-sm font-bold mb-2">Phương thức thanh toán:</label>
                <input type="text" name="PaymentMethod" id="PaymentMethod" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('PaymentMethod') }}">
            </div>

            <h2 class="text-xl font-semibold text-gray-800 mb-3 mt-6">Chi tiết Đơn hàng</h2>
            <div id="order-items-container">
                {{-- Các dòng sản phẩm sẽ được thêm vào đây bằng JavaScript hoặc lặp qua old input --}}
                @if (old('products'))
    ...
    <script>
        let itemIndex = {{ old('products') ? count(old('products')) : 0 }};
